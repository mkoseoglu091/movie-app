...
<div class="card flex-row mx-3 bg-light">
  ...
  <h5 class="card-title h1 h2-sm"> {{ $viewData["movie"]["title"] }}</h5>
    <p class="card-text"> 
    <a href="{{ '/scenes/'.$viewData['user_id'].'/'.$viewData['movie']['id'] }}" class="btn btn-dark">Back</a>

    <form method="POST" action="{{ route('scene.update', ['id' => $viewData['movie']->id, 'user_id' => $viewData['movie']->user_id, 'scene_id' => $viewData['scene']->id]) }}" enctype="multipart/form-data">
    @csrf
    <div class="col-md-4">
