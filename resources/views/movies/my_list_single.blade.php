...
<div class="card flex-row mx-3 bg-light">
  ...
  <h5 class="card-title h1 h2-sm"> {{ $viewData["movie"]["title"] }}</h5>
    ...
    <a href="{{ route('movie.edit', $viewData['id']) }}" style="float:left;" class="btn btn-primary">Edit</a>
    
    <form class="mx-3" style="float:left;" action = "{{ route('movie.delete', $viewData['movie']['id']) }}" method="POST">
      @csrf
      @method('DELETE')
